...
public class JwtTokenHandler {
    ...
    public User getClaimAndTokenNotExpired(String token){
        Claims body = getClaimFromToken(token);
        if(body.getExpiration().after(new Date())){
            return transformFromToken(body);
        }
        return null;
    }
    ...
    /**
     * @author 董感恩
     * @date 2020-02-06 17:05:45
     * @desc 把负载转换成对象
     **/
    public User transformFromToken(Claims claims){
        User user = new User();
        user.setUsername(claims.getSubject());
        user.setPermissionServices((List<String>) claims.get("permissionServices"));
        return user;
    }
}